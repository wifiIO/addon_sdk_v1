//全局变量，用于维护配置信息
glb_addon_name = "stcps"
glb_json_cfg_file_path = "/app/"+glb_addon_name+"/cfg.json"
glb_statue_invoke_target = "stcps.status"
...
$(document).ready(
	function(){
		...
		setTimeout(sys_status_info_update, 500);	//通过invoke接口 读取状态json的数据
	...
	function sys_status_info_update()
	{
		...
		$.ajax({
			...
			success: function (res_js){
				...
				$("#stcps_status_up_byte").html(res_js.up_byte);
				$("#stcps_status_dn_byte").html(res_js.dn_byte);
				$("#stcps_status_connection").html(res_js.connection);
			}
		});
	}
